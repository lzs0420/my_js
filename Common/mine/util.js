/**
 * Created by Allen on 2016/03/09.
 */
    ...
    for ( var i = 0; i < form.elements.length; i++){
        ...
        if(tagName ==='INPUT'||tagName === 'TEXTAREA'){var type = element.type;
            if ((type === 'text')||(type === 'password') || (type === 'hidden') || (tagName === 'TEXTAREA')){
                sb.append("\""+name+"\":\""+encodeURIComponent(encodeURIComponent(element.value.replace(/\r\n/ig,"")))+"\"");
            }else if((type === 'checkbox') || (type === 'radio')){
                rcs.putOverride(name,type);
            }else{continue;}
        }else if (tagName === 'SELECT'){var oc = element.options.length;
            for ( var j = 0; j <oc; j++){
                if (element.options[j].selected){sb.append("\""+name+"\":\""+(encodeURIComponent(encodeURIComponent(element.value.replace(/\r\n/ig,""))))+"\"");}
            }
        }
    }
    if(rcs.size()>0){
        for(var i=0;i<rcs.size();i++){
            ...
            sb.append("\""+r.key+"\":\""+encodeURIComponent(encodeURIComponent(radio.replace(/\r\n/ig,"")))+"\"");
        }
    }
...
function form2Param(formId){
    ...
    return sb.toParamString();
}

function run(page,formId,callbackFunction){
    var json = '{"RequestParams":'+form2Json(formId)+',"deviceType":"Pc"}';
    var result= $.ajax({
        type: 'POST',
        url: page,
        data: "param="+json,
